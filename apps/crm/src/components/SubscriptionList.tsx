    ...
    <div role="main">
      <div className="p-20 bg-slate-700 h-screen">
        ...
        <div className="bg-white shadow-md rounded-lg p-6 space-y-2">
          {subscriptions.map((subscription) => (
            <SubscriptionItem key={subscription.id} subscription={subscription} onOpenModal={handleOpenModal} />
          ))}
        </div>
      </div>
      <Modal isOpen={isModalOpen} onClose={() => setModalOpen(false)}>
        <div className="space-y-4">
          ...
          <button className="bg-blue-500 text-white px-4 py-2 rounded-md">Try new experience</button>
        </div>
      </Modal>
    </div>
  );
